...
export class AddToCartPage {
    ...
    for (let section in sections) {
      ...
      if (section === sectionTitle) {
        ...
        const link = this.page.locator(
          `${sections[section]} > .thumbnails > ${element} > .thumbnail > .pricetag > a`
        );
        ...
        if ((await link.getAttribute("href")) !== "#") {
          ...
          await this.page
            .locator(
              `${sections[section]} > .thumbnails > ${element} > .thumbnail > .pricetag > .price > .oneprice`
            )
            .textContent();
          ...
          //Click on Add to cart
          const addedToCart = this.page
            .locator(
              `${sections[section]} > .thumbnails > ${element} > .thumbnail > .pricetag.added_to_cart > .quick_basket > a`
            )
            .last();
          //Click on added to cart
          await addedToCart.click();
          //Call getValuesFromCheckoutTable function
          let noHrefValuesFromTable =
            await this.getValuesFromCheckoutTable(productTitle);
          return {
  ...
  async getTotalPrice() {
    //Sub total text
    const subTotal: any = await this.page
      .locator("#totals_table > tbody > tr > td")
      .nth(1)
      .locator("span")
      .textContent();
    ...
    const flatShippingRate: any = await this.page
      .locator("#totals_table > tbody > tr > td")
      .nth(3)
      .locator("span")
      .textContent();
    //Formatted flat shipping rate text
    const flatShippingRateFormatted = flatShippingRate?.replace(
      /^(-)|[^0-9.,]+/g,
      ""
    );
    //Total price locator
    const totalPrice = this.page
      .locator("#totals_table > tbody > tr > td")
      .nth(5);
    //Total price text
    ...
    if (
      parseFloat(subTotalFormatted) + parseFloat(flatShippingRateFormatted) ===
  ...
  async getValuesFromCheckoutTable(productTitle: string) {
    ...
    let row = this.page
      .locator(".table.table-striped.table-bordered tbody")
      .locator("tr");
    //Get count of rows
    ...
    for (let i = 1; i < rowCount - 1; i++) {
      ...
      let quantityValue = await row
        .nth(i)
        .getByRole("cell")
        .locator("div > input.form-control.short")
        .inputValue();
    ...
    for (let i = 0; i < productArray.length; i++) {
      if (productArray[i][1] === productTitle) {
        ...
        //Get subtotal price
        let subTotalPrice = (
          parseFloat(priceUnit) * parseFloat(quantity[i])
        ).toFixed(2);
        return {
